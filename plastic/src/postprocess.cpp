...
{
    namespace surface_t {
        int get_tex_id(Value* v) { return as_int(v->getIndex(0)); }
        int get_fbo_id(Value* v) { return as_int(v->getIndex(1)); }
        int get_width(Value* v) { return as_int(v->getIndex(2)); }
        int get_height(Value* v) { return as_int(v->getIndex(3)); }

        void set_tex_id(Value* v, int id) { touch(v); set_int(v->getIndex(0), id); }
        void set_fbo_id(Value* v, int id) { touch(v); set_int(v->getIndex(1), id); }
    ...
    void bind_surface(EvalContext* cxt, Term* caller, Value* surface)
    {
    ...
    {
        Value* source_surface = INPUT(0);
        Value* dest_surface = INPUT(1);
