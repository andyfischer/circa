...
namespace math_function {
    ...
    {
        make_float(OUTPUT, fmodf(FLOAT_INPUT(0), FLOAT_INPUT(1)));
    }
    ...
    CA_DEFINE_FUNCTION(mod_f, "mod_f(number,number) -> number")
    {
        ...
        if (a >= 0)
            make_float(OUTPUT, fmodf(a, n));
        else
            make_float(OUTPUT, fmodf(a, n) + n);
    }
