...
void test_recursion()
{
    ...
    TValue* fact_1 = branch.eval("factorial(1)");
    test_assert(&branch);
    test_assert(fact_1->asInt() == 1);

    TValue* fact_2 = branch.eval("factorial(2)");
    test_assert(&branch);
    test_assert(fact_2->asInt() == 2);

    TValue* fact_3 = branch.eval("factorial(3)");
    test_assert(&branch);
    test_assert(fact_3->asInt() == 6);

    TValue* fact_4 = branch.eval("factorial(4)");
    test_assert(&branch);
...
void stateful_function_with_arguments()
{
    ...
    TValue* call = branch.eval("myfunc(5)");
...
namespace copy_counting_tests
{
    ...
    void t_initialize(Type* type, TValue* source)
    {
        source->value_data.asint = next_available_slot++;
    }

    void t_copy(Type* type, TValue* source, TValue* dest)
    {
