...
{
    Responder* responder = (Responder*) circa_raw_pointer(value);
...
{
    Requester* requester = (Requester*) circa_raw_pointer(value);
...
{
    void* socket = (Requester*) circa_raw_pointer(value);
    zmq_close(socket);
}
...
void zmsg_to_cavalue(zmq_msg_t* msg, caValue* value)
{
    ...
    circa_parse_string(circa_string(&str), value);
}

int cavalue_to_zmsg(caValue* value, zmq_msg_t* msg)
{
    circa::Value str;
    circa_to_string(value, &str);
    zmq_msg_init_from_str(msg, circa_string(&str));
    return circa_string_length(&str);
}

void create_responder(caStack* stack)
{
    printf("create_responder called\n");
    ...
    if (zmq_bind(responder->socket, addr) == -1) {
        ...
        circa_output_error(stack, "zmq_bind failed\n");
        delete responder;
        return;
    }

    caValue* out = circa_create_default_output(stack, 0);
    circa_set_raw_pointer(circa_index(out, 0), (void*) responder);
}

void Responder_read(caStack* stack)
{
    Responder* responder = (Responder*) circa_raw_pointer(circa_index(circa_input(stack, 0), 0));

    // Don't allow a read if we never replied to the last message
    if (responder->expectingReply) {
        circa_output_error(stack, "read() called but we never sent a reply() to previous message");
        return;
    }

    zmq_msg_t msg;
    zmq_msg_init(&msg);
    if (zmq_recv(responder->socket, &msg, 0, ZMQ_NOBLOCK) == -1) {
        if (errno == EAGAIN) {
...
void Responder_reply(caStack* stack)
{
    Responder* responder = (Responder*)  circa_raw_pointer(circa_index(circa_input(stack, 0), 0));

    if (!responder->expectingReply) {
        circa_output_error(stack, "reply() called but there's no message to reply to");
        return;
    }
    ...
    zmq_msg_t msg;
    int size = cavalue_to_zmsg(circa_input(stack, 1), &msg);

    if (zmq_send(responder->socket, &msg, size, 0) == -1) {
        printf("zmq_send failed with error: %s\n", strerror(errno));
    }
    zmq_msg_close(&msg);
}

void create_requester(caStack* stack)
{
    ...
    caValue* out = circa_create_default_output(stack, 0);
    circa_set_raw_pointer(circa_index(out, 0), (void*) requester);
}

void Requester_send(caStack* stack)
{
    Requester* requester = (Requester*) circa_raw_pointer(circa_index(circa_input(stack, 0), 0));

    zmq_msg_t msg;
    int size = cavalue_to_zmsg(circa_input(stack, 1), &msg);

    if (zmq_send(requester->socket, &msg, size, 0) == -1) {
        printf("zmq_send failed with error: %s\n", strerror(errno));
        circa_output_error(stack, "zmq_send failed");
    }
    zmq_msg_close(&msg);
}

void Requester_receive(caStack* stack)
{
    Requester* requester = (Requester*) circa_raw_pointer(circa_index(circa_input(stack, 0), 0));
    ...
    if (zmq_recv(requester->socket, &msg, 0, 0) == -1) {
        printf("zmq_recv failed with error: %s\n", strerror(errno));
        circa_output_error(stack, "zmq_recv failed");
...
void create_publisher(caStack* stack)
{
    int port = circa_int_input(stack, 0);
    ...
    caValue* out = circa_create_default_output(stack, 0);
    circa_set_raw_pointer(circa_index(out, 0), (void*) socket);
}

void Publisher_send(caStack* stack)
{
    void* socket = circa_raw_pointer(circa_index(circa_input(stack, 0), 0));

    zmq_msg_t msg;
    int size = cavalue_to_zmsg(circa_input(stack, 1), &msg);

    if (zmq_send(socket, &msg, size, 0) == -1) {
        printf("zmq_send failed with error: %s\n", strerror(errno));
    }
    zmq_msg_close(&msg);
}

void create_subscriber(caStack* stack)
{
    const char* addr = circa_string_input(stack, 0);
    ...
    if (zmq_connect(socket, addr) == -1) {
        ...
        circa_output_error(stack, "zmq_connect failed");
        zmq_close(socket);
        return;
    }
...
void Subscriber_poll(caStack* stack)
{
    void* socket = circa_raw_pointer(circa_index(circa_input(stack, 0), 0));
    ...
    if (zmq_recv(socket, &msg, 0, ZMQ_NOBLOCK) == -1) {
        if (errno == EAGAIN) {
            // No message available right now; this is normal.
            circa_set_null(circa_output(stack, 0));
        } else {
...
namespace zmq_file_source {
    ...
    void zmq_init_file_source(const char* addr, caValue* error)
    {
        ...
        for (int i=0; i < circa_list_length(file_list); i++) {
            ...
            circa_fetch_file_record(circa_string(item), fileSource.name);
        }

        // Connect subscriber socket
        client->subscriberSocket = zmq_socket(g_context, ZMQ_SUB);
        
        if (zmq_bind(client->subscriberSocket, circa_string(publisher_addr)) == -1) {
    ...
    bool read_next_feed_msg(ZmqFileClient* client)
    {
        ...
        if (circa_string_eq(eventType, "update")) {
            ...
            caFileRecord* record = circa_get_file_record(circa_string(filename));

            if (record != NULL) {
                free(record->data);
                record->data = strdup(circa_string(contents));
                record->version++;
            }
    ...
    void file_source_update_file(caFileSource* source, caFileRecord* record)
    {
        ...
        if (record->data == NULL) {
            ...
            zmq_msg_t zmsg;
            zmq_msg_init_from_str(&zmsg, circa_string(&requestStr));
            ...
            record->data = strdup(circa_string(&reply));
        }
    }
}
#endif
...
{
    circa_patch_type_release(module, "nativeResponder", ResponderRelease);
    circa_patch_type_release(module, "nativeRequester", RequesterRelease);
    circa_patch_type_release(module, "nativeSocket", SocketRelease);
    circa_patch_function(module, "create_responder", create_responder);
    circa_patch_function(module, "Responder.read", Responder_read);
    circa_patch_function(module, "Responder.reply", Responder_reply);
    circa_patch_function(module, "create_requester", create_requester);
    circa_patch_function(module, "Requester.send", Requester_send);
    circa_patch_function(module, "Requester.receive", Requester_receive);
    circa_patch_function(module, "create_publisher", create_publisher);
    circa_patch_function(module, "Publisher.send", Publisher_send);
    circa_patch_function(module, "create_subscriber", create_subscriber);
    circa_patch_function(module, "Subscriber.poll", Subscriber_poll);
